...
int main(int argc, char *argv[])
{
	...
	if (my_file.is_open())
	{
		while(getline(my_file,line))// store each line from the file into the array
		{
			...
			if(num_of_proc != line_counter-1) // error checking: number of processes should be equal to the number of process in txt file
			{
		...
		if(rank == coordinator) // the coordinator process
		{
			for(int i=0; i < num_of_proc; i++)
			{
					...
					MPI_Send(&value, 1, MPI_FLOAT, i, 33, MPI_COMM_WORLD); // send time to be adjusted to respective processe	
			}
